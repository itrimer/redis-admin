...
//		RedisZtreeUtil.initRedisNavigateZtree(name);
    ...
    private RedisClusterConfiguration getRedisClusterConfiguration(String host) {
    ...
    public CopyOnWriteArrayList<RKey> iss(RedisTemplate redisTemplate, String serverName, int dbIndex, String key) {
        RedisConnection connection = RedisConnectionUtils.getConnection(redisTemplate.getConnectionFactory());
        try {
            connection.select(dbIndex);
            ...
            ConvertUtil.convertByteToString(connection, keysSet, tempList);
            Collections.sort(tempList);
            CopyOnWriteArrayList<RKey> redisKeysList = new CopyOnWriteArrayList<>(tempList);
