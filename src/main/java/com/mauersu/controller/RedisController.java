...
public class RedisController extends RedisApplication implements Constant{
	...
	public Object stringList(HttpServletRequest request, HttpServletResponse response,
		...
		String queryMode = StringUtil.getParameterByDefault(request, "queryMode", MIDDLE_KEY);
		...
		Set<RKey> redisKeys = viewService.getRedisKeys(pagination, serverName, dbIndex, keyPrefixs, queryMode, queryValue);
	...
	private Pagination stringListPagination(HttpServletRequest request, String queryMode, String queryMode_ch, String queryValue, String queryByKeyPrefixs) {
		Pagination pagination = getPagination(request);
		String url = "?" + "queryMode=" + queryMode + "&queryMode_ch=" + queryMode_ch + "&queryValue=" + queryValue + "&keyPrefixs="+queryByKeyPrefixs;
